...
class SupportTicketController extends GetxController {
  ...
  Future getSupportTickets() async {
    try {
      ...
      supportTickets.value = supportTicketModelFromJson(response.body.toString());
      log("response=${supportTickets.value}");
  ...
  Future getTicketCategories() async {
    try {
      ...
      log("====${response.body}");
      ticketCategories.value = ticketCategoriesFromJson(response.body.toString());
