...
class _SupportTicketsPageState extends State<SupportTicketsPage> {
  ...
  Widget build(BuildContext context) {
    print("controller.supportTickets.value : ${controller.supportTickets.value}");
    return Obx(() {
            ...
            body: TabBarView(
              children: List.generate(controller.supportTickets.value.statuses.length + 1, (index) {
                ...
                return SupportTicketByStatusPage(
                    statusId: controller.supportTickets.value.statuses[index - 1].id,
                    statusName: controller.supportTickets.value.statuses[index - 1].name);
              }),
            ),
          ),
        );
